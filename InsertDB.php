...
function Verifica($NProtocolo){
	include "connect.php";
	$sql = "SELECT NProtocolo, TServico FROM tbl_telefone where NProtocolo = $NProtocolo";
	$result = $conn->query($sql);
	$check = true;
	while($row = $result->fetch_assoc()) {
		if ($row["NProtocolo"] == $NProtocolo){
			$check = false;
		}
	}
	return $check;	
